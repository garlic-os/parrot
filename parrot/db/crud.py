...
class CRUD:
	...
	def guild_get_channel_ids_with_permission(
	...
	def channel_set_permission_flag(
		...
		"""
		statement = sqlmodel.select(p.Channel).where(p.Channel.id == channel.id)
		db_channel = self.session.exec(statement).first()
	...
	def get_prefix(self, guild: discord.Guild) -> str:
		statement = sqlmodel.select(p.Guild.imitation_prefix).where(
			p.Guild.id == guild.id
		)
		return (
	...
	def set_prefix(self, guild: discord.Guild, new_prefix: str) -> None:
		statement = sqlmodel.select(p.Guild).where(p.Guild.id == guild.id)
	...
	def get_suffix(self, guild: discord.Guild) -> str:
		statement = sqlmodel.select(p.Guild.imitation_suffix).where(
			p.Guild.id == guild.id
		)
		return (
			self.session.exec(statement).first()
